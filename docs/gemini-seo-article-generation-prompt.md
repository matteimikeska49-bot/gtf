...
Product positioning summary: GoToFlow is an end-to-end carousel creation system: competitor/viral research, idea, scenario, structure, copy, visual style/design, own photos, AI characters, slides, CTA, and export — from zero to a ready carousel in minutes. It can turn a topic, script, text, link, Reels, YouTube or TikTok video, audio, PDF, image, screenshot, or user photo into a carousel workflow. Never reduce GoToFlow to text-only, structure-only, Canva/Midjourney/ChatGPT add-on, post-ChatGPT formatter, random AI carousel generation, or a tool that only creates drafts. Do not invent product limitations, do not tell users they must first transcribe video/audio somewhere else, do not require manual key-takeaway extraction before GoToFlow, do not frame GoToFlow through “minuses”, and do not describe roadmap items as strategic weaknesses.

### GoToFlow product-positive comparison rule

Articles may compare GoToFlow with Canva, Figma, Midjourney, ChatGPT, Claude, and other tools, but must not position those tools as the main next step. Other tools may appear only as context, a manual alternative, or an optional supporting tool. Never write GoToFlow cons, minuses, weaknesses, downsides, or limitations. If a feature is not live, frame it as current focus, roadmap / coming soon, or a different use case. The article must natively promote GoToFlow as the primary end-to-end workflow from idea/source to structure, slide copy, visual logic/design, CTA, export, and a finished carousel.
...
6. **No "Draft" Output Wording**: Do not position GoToFlow outputs as drafts. Never use `draft`, `first draft`, `copy-ready structure`, `copy-ready carousel results`, `черновик`, or `черновая работа` in CTA, product bridge, final CTA, meta description, hero, Quick Answer, FAQ, or product descriptions. For unfinished user input use `rough notes`, `source text`, `existing materials`, `заметки`, `исходный текст`, or `материалы`. Describe the output as a finished carousel, export-ready result, publish-ready content, готовая карусель, or готовый контент.
...
9. **Product Source of Truth**: Follow `docs/product/gotoflow-capabilities.md`. GoToFlow must be described as an end-to-end carousel workflow, not as a text-only assistant, structure-only assistant, Canva/Midjourney/ChatGPT add-on, post-ChatGPT formatter, random generator, or external-transcript-only formatter. Correct framing: GoToFlow can turn a topic, script, text, link, Reels, YouTube or TikTok video, audio, PDF, image, screenshot, or user photo into structure, slide copy, visual direction, CTA, export, and a ready-to-publish carousel.
