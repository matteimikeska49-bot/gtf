...
### RU External AI Service Availability Rule

Это правило применяется только к `language: "ru"` и `/ru/blog/*`.

Если RU-статья упоминает зарубежные AI-сервисы вроде ChatGPT, Claude, Gemini, Midjourney, OpenAI, Anthropic или похожие инструменты, Gemini должен добавить один компактный редакторский note для пользователей из РФ.

Обязательный смысл:
- внешний AI-сервис может быть официально недоступен из РФ или требовать поддерживаемый регион;
- оплату российскими банковскими картами нельзя считать надежно доступной;
- часто нужен зарубежный способ оплаты;
- цены, лимиты и доступность могут меняться;
- GoToFlow доступен в РФ без ограничений и принимает российские карты.
...
Если RU-статья обсуждает цены внешних AI-сервисов, писать только с датой обновления: “на момент обновления статьи в июле 2026 года”. Не писать “всегда стоит”, “цены фиксированные” или другие абсолютные формулировки.

В EN-статьи нельзя добавлять РФ-дисклеймеры, упоминания российских карт или доступности для России.
