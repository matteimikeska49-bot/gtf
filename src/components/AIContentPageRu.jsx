    ...
    <div className="max-w-7xl mx-auto flex flex-col items-center text-center relative z-10 w-full">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="mt-4 md:mt-0 mb-8">
        <div className="inline-flex items-center gap-2.5 rounded-full border border-white/10 bg-white/5 px-4 py-1.5 backdrop-blur-sm">
          ...
          <span className="text-sm text-zinc-300 whitespace-nowrap">Генератор контента с ИИ</span>
        </div>
      </motion.div>
      ...
      <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: isMobile ? 0.6 : 0.8, delay: 0.2 }} className="text-sm md:text-[0.92rem] text-zinc-500 max-w-lg mx-auto mb-12 leading-[1.75] font-medium text-balance">
        От идеи до готовых слайдов — структура, текст и логика контент генерируются автоматически.<br className="hidden md:block" /> Без навыков дизайна, без команды, без шаблонов.
      </motion.p>
...
/* ── Showcase (RU) ── */
const carouselCards = [
  { id: 2, likes: '8.1K', views: '20K', label: 'Контент пример 2', image: '/images/niches/ru/content-ru-2.webp' },
  { id: 3, likes: '1.8K', views: '5K', label: 'Контент пример 3', image: '/images/niches/ru/content-ru-3.webp' },
  { id: 5, likes: '6.7K', views: '18K', label: 'Контент пример 5', image: '/images/niches/ru/content-ru-5.webp' },
  { id: 6, likes: '5.9K', views: '14K', label: 'Контент пример 6', image: '/images/niches/ru/content-ru-6.webp' },
  { id: 7, likes: '2.4K', views: '7K', label: 'Контент пример 7', image: '/images/niches/ru/content-ru-7.webp' },
  { id: 8, likes: '980', views: '3.2K', label: 'Контент пример 8', image: '/images/niches/ru/content-ru-8.webp' },
  { id: 9, likes: '10K', views: '25K', label: 'Контент пример 9', image: '/images/niches/ru/content-ru-9.webp' },
  { id: 10, likes: '7.2K', views: '15K', label: 'Контент пример 10', image: '/images/niches/ru/content-ru-10.webp' },
];
...
/* ── Problem (RU) ── */
const problemCards = [
  { title: "Тратите 3 часа → один материал", text: "Написать хуки, продумать структуру и подготовить текст занимает много времени — особенно если результат обычного ИИ приходится полностью переписывать." },
  { title: "Первый слайд не останавливает скролл", text: "Слабые хуки — и никто не свайпает дальше. Контент умирает на первом слайде, даже если дальше всё хорошо." },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full mb-12">
              ...
              <div className="rounded-2xl p-6 md:p-8 flex flex-col h-full relative group transition-colors duration-500" style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', boxShadow:'0 20px 40px -10px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.06)', backdropFilter:'blur(16px)' }}>
                ...
                <div className="flex flex-col gap-5 flex-1 relative z-10">
                  {['Введите тему или вставьте ссылку — получите готовую контент','ИИ пишет хук, структурирует слайды и заполняет текст','Получите готовую к публикации контент в один клик','Мгновенно генерируйте несколько вариантов','Ваш tone of voice сохраняется в каждом слайде'].map((t,i)=>(
...
const ExportMockup = () => (
  <div className="mt-6">
    <button className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-pink-500/50 bg-pink-500/10 shadow-[0_0_20px_rgba(236,72,153,0.2)] text-pink-300 text-xs font-semibold tracking-wide transition-all hover:shadow-[0_0_28px_rgba(236,72,153,0.4)] group">
      <Download className="w-3.5 h-3.5 group-hover:-translate-y-0.5 transition-transform duration-300"/> Скачать контент
    </button>
  </div>
);

const hiwSteps = [
  { icon: CornerDownLeft, number: 1, title: 'Введите тему или ссылку', desc: 'Введите тему, URL или идею. ИИ изучит контекст и подготовит контент.', micro: <InputMockup/> },
  { icon: Sparkles, number: 2, title: 'ИИ создаёт контент', desc: 'ИИ собирает хук, структуру, текст, визуальную подачу и CTA.', micro: <ProgressMockup/> },
  { icon: Download, number: 3, title: 'Получите готовые слайды', desc: 'Проверьте, отредактируйте при необходимости и скачайте готовую контент.', micro: <ExportMockup/> },
];
    ...
    <motion.div initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.7,delay}} className={`relative group transition-transform duration-500 hover:-translate-y-1 transform-gpu ${className}`}>
      ...
      <div className="relative z-10 p-2 md:p-3 bg-[#ffffff03] backdrop-blur-2xl border border-white/[0.08] shadow-[0_20px_50px_rgba(0,0,0,0.5)] rounded-[1.25rem] flex flex-col justify-center h-full group-hover:border-white/[0.14] transition-colors duration-500">
        <div className="relative rounded-[0.75rem] overflow-hidden border border-white/[0.03] bg-[#0c0508] flex-grow flex items-center justify-center">
          {!err ? <img src={`/images/screens/screen-${imageId}.png`} alt={`GoToFlow AI генератор контента экран ${imageId}`} className="w-full h-auto block object-contain transition-transform duration-700 group-hover:scale-[1.02]" onError={()=>setErr(true)}/> :
    ...
    <div className="max-w-7xl mx-auto relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-20 items-center">
      <div className="flex flex-col">
        <motion.div initial={{opacity:0,x: isMobile ? 0 : -40, y: isMobile ? 24 : 0}} whileInView={{opacity:1,x:0,y:0}} viewport={{once:true}} transition={{duration: isMobile ? 0.6 : 0.8}}>
          ...
          <p className="text-base md:text-lg text-zinc-400 font-medium leading-relaxed text-balance mb-12">Пока другие сервисы дают вам пустой холст, GoToFlow генерирует полную контент — структуру слайдов, тексты и логику — используя ИИ.</p>
        </motion.div>
  ...
  <section className="py-20 md:py-28 px-6 relative z-10 w-full bg-[#050505]">
    <div className="max-w-3xl mx-auto">
      ...
      <div className="text-zinc-400 leading-relaxed space-y-4 text-base">
        ...
        <p>Используя <strong className="text-zinc-200">генератор контента для соцсетей</strong>, вы ускоряете производство, получаете стабильное качество текстов и можете легко тестировать разные гипотезы. Будь вы эксперт, маркетолог или фаундер, GoToFlow помогает пройти путь от исходного материала до готового контента для публикации.</p>
      </div>
    </div>
  </section>
);
