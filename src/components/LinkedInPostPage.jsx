...
export const SEOHead = () => {
  useEffect(() => {
    document.title = 'AI LinkedIn Post Generator | GoToFlow';
    ...
    setMeta('title', 'AI LinkedIn Post Generator | GoToFlow');
    setMeta('description', 'Generate LinkedIn posts, captions, and ideas instantly with AI.');
    setMeta('og:title', 'AI LinkedIn Post Generator | GoToFlow', true);
    setMeta('og:description', 'Generate LinkedIn posts, captions, and ideas instantly with AI.', true);
    setMeta('og:url', 'https://gotoflow.io/ai-linkedin-post-generator', true);
    setMeta('twitter:title', 'AI LinkedIn Post Generator | GoToFlow', true);
    setMeta('twitter:description', 'Generate LinkedIn posts, captions, and ideas instantly with AI.', true);
    setMeta('twitter:url', 'https://gotoflow.io/ai-linkedin-post-generator', true);
    
    setLink('canonical', 'https://gotoflow.io/ai-linkedin-post-generator');
    setLink('alternate', 'https://gotoflow.io/ai-linkedin-post-generator', { hreflang: 'en' });
    setLink('alternate', 'https://gotoflow.io/ru/generator-postov-linkedin', { hreflang: 'ru' });
    setLink('alternate', 'https://gotoflow.io/ai-linkedin-post-generator', { hreflang: 'x-default' });
    ...
    <div className="max-w-7xl mx-auto flex flex-col items-center text-center relative z-10 w-full">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="mt-4 md:mt-0 mb-8">
        <div className="inline-flex items-center gap-2.5 rounded-full border border-white/10 bg-white/5 px-4 py-1.5 backdrop-blur-sm">
          ...
          <span className="text-sm text-zinc-300 whitespace-nowrap">AI LinkedIn Post Generator for creators and teams</span>
        </div>
      </motion.div>
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: isMobile ? 0.6 : 0.8, delay: 0.1 }} className="max-w-3xl mx-auto w-full">
        <h1 className="text-[1.6rem] sm:text-[2rem] md:text-[2.6rem] lg:text-[3.1rem] font-bold text-white tracking-[-0.035em] leading-[1.12] mb-8 text-balance">
          AI LinkedIn Post Generator —<br className="hidden md:block" /> Create Posts in<br className="hidden md:block" /> <span className="text-gradient-brand">60 Seconds</span>
        </h1>
      </motion.div>
      <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: isMobile ? 0.6 : 0.8, delay: 0.2 }} className="text-sm md:text-[0.92rem] text-zinc-500 max-w-lg mx-auto mb-12 leading-[1.75] font-medium text-balance">
        Generate a complete LinkedIn or LinkedIn post in seconds — from idea to final slides.<br className="hidden md:block" /> No design skills, no team, no templates.
      </motion.p>
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: isMobile ? 0.6 : 0.8, delay: 0.3 }} className="flex flex-col items-center gap-4 w-full sm:w-auto">
        <button onClick={() => window.location.href = getAppUrlWithRef(CTA_URL)} className="w-full sm:w-auto px-8 py-4 rounded-full font-bold text-white bg-gradient-to-r from-pink-500 to-orange-500 transition-all hover:scale-105 hover:shadow-[0_0_60px_rgba(236,72,153,0.5)] active:scale-[0.98] shadow-[0_0_40px_rgba(236,72,153,0.4)] flex items-center justify-center gap-2 group text-base border border-pink-400/20 z-20 relative">
          Create Your Post <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
        </button>
        <p className="text-sm text-zinc-500 flex flex-wrap justify-center gap-x-3 gap-y-1"><span>✓ No credit card required</span><span className="text-zinc-700">•</span><span>✓ First post in 60 seconds</span></p>
      </motion.div>
    </div>
  </section>
);
...
/* ── Showcase (post-only) ── */
const postCards = [
  { id: 2, format: 'reel', likes: '8.1K', views: '20K', label: 'Post example 2', image: '/images/niches/en/content-en-2.webp' },
  { id: 3, format: 'reel', likes: '1.8K', views: '5K', label: 'Post example 3', image: '/images/niches/en/content-en-3.webp' },
  { id: 5, format: 'reel', likes: '6.7K', views: '18K', label: 'Post example 5', image: '/images/niches/en/content-en-5.webp' },
  { id: 6, format: 'reel', likes: '5.9K', views: '14K', label: 'Post example 6', image: '/images/niches/en/content-en-6.webp' },
  { id: 7, format: 'reel', likes: '2.4K', views: '7K', label: 'Post example 7', image: '/images/niches/en/content-en-7.webp' },
  { id: 8, format: 'reel', likes: '980', views: '3.2K', label: 'Post example 8', image: '/images/niches/en/content-en-8.webp' },
  { id: 9, format: 'reel', likes: '10K', views: '25K', label: 'Post example 9', image: '/images/niches/en/content-en-9.webp' },
  { id: 10, format: 'reel', likes: '7.2K', views: '15K', label: 'Post example 10', image: '/images/niches/en/content-en-10.webp' },
];
...
const SlideCard = ({ card }) => (
  <div className="shrink-0 w-[280px] md:w-[320px] bg-white/[0.02] border border-white/[0.06] rounded-2xl p-3 flex flex-col gap-3">
    <div className="relative w-full aspect-[4/5] rounded-xl bg-[#111] overflow-hidden">
      ...
      <span className="absolute top-2.5 left-2.5 text-[10px] font-bold px-2.5 py-1 rounded-full z-[2] bg-violet-500/20 text-violet-300">Post</span>
    </div>
    ...
    <div className="relative z-10">
      <div className="flex justify-center mb-6">
        <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full border border-white/10 bg-white/[0.03] backdrop-blur-md shadow-lg shadow-black/20">
          ...
          <span className="text-sm text-zinc-300 font-medium tracking-tight"><span className="text-white font-semibold">+10k</span> already creating posts</span>
        </div>
      </div>
      <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white text-center tracking-tight mb-16 px-6">
        Real Posts <span className="text-gradient-brand">Generated with AI</span>
      </h2>
      <div className="relative overflow-hidden" style={{ maskImage: 'linear-gradient(to right, transparent 0%, black 10%, black 90%, transparent 100%)', WebkitMaskImage: 'linear-gradient(to right, transparent 0%, black 10%, black 90%, transparent 100%)' }}>
        <div className="flex items-center gap-5" style={{ animation: 'marquee-scroll 35s linear infinite', width: 'max-content', willChange: 'transform' }}>
          {[...postCards, ...postCards].map((card, i) => <SlideCard key={`${card.id}-${i}`} card={card} />)}
        </div>
      </div>
    </div>
...
/* ── Problem ── */
const problemCards = [
  { title: "Spend 3 hours → get one post", text: "Writing hooks, structuring slides, and formatting takes forever — even with generic AI output that still needs full rewriting." },
  { title: "First slide doesn't stop the scroll", text: "Weak hooks mean nobody swipes. Your post dies on slide 1 no matter how good the rest is." },
  ...
  return (
    <section ref={ref} className="relative z-10 py-24 md:py-32 w-full flex flex-col items-center bg-[#050505]">
      ...
      <motion.h2 initial={{ opacity:0,y:12 }} animate={inView?{opacity:1,y:0}:{}} transition={{ duration:0.7 }} className="text-4xl md:text-5xl font-bold text-white mb-4 tracking-tight text-center relative z-20 px-6">
        Post creation turned into <span className="text-transparent bg-clip-text bg-gradient-to-r from-pink-500 via-rose-500 to-orange-400">chaos?</span>
      </motion.h2>
    ...
    <div className="max-w-6xl mx-auto relative z-10">
      <motion.div initial={{ opacity:0,y:32 }} whileInView={{ opacity:1,y:0 }} viewport={{ once:true }} transition={{ duration:0.7 }} className="text-center mb-14">
        <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white tracking-tight mb-5">
          Old post workflow vs <span className="text-transparent bg-clip-text bg-gradient-to-r from-pink-500 via-rose-500 to-orange-400">GoToFlow</span>
        </h2>
        <p className="text-lg md:text-xl text-zinc-400 max-w-3xl mx-auto leading-relaxed font-medium">Idea sourcing, hook writing, slide structuring, and copywriting usually happen in 4+ different tools. In GoToFlow, it's one seamless post creation flow.</p>
      </motion.div>
      <motion.div initial={{ opacity:0,y:56 }} whileInView={{ opacity:1,y:0 }} viewport={{ once:true }} transition={{ duration: isMobile ? 0.6 : 0.9 }} className="relative transform-gpu">
        <div className="relative rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-12 lg:p-14">
          ...
          <div className="relative z-10 flex flex-col">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12 max-w-4xl mx-auto w-full">
              {[{ icon:<Clock className="w-6 h-6"/>, color:'text-amber-400', ring:'bg-amber-500/10 border-amber-500/20', text:'Save 5+ hours per week on posts' },
                { icon:<Zap className="w-6 h-6"/>, color:'text-violet-400', ring:'bg-violet-500/10 border-violet-500/20', text:'Ready post in ~60 seconds' },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full mb-12">
              <div className="rounded-2xl p-6 md:p-8 flex flex-col h-full" style={{ background:'rgba(5,5,5,0.6)', border:'1px solid rgba(255,255,255,0.03)', boxShadow:'inset 0 4px 24px rgba(0,0,0,0.4)' }}>
                ...
                <div className="flex flex-col gap-5 flex-1">
                  {['Writing hooks manually in ChatGPT','Structuring slides in a separate doc','Designing in Canva from scratch every time','Getting raw AI text and rewriting slide by slide','No system — every post starts from zero'].map((t,i)=>(
              ...
              <div className="rounded-2xl p-6 md:p-8 flex flex-col h-full relative group transition-colors duration-500" style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', boxShadow:'0 20px 40px -10px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.06)', backdropFilter:'blur(16px)' }}>
                ...
                <div className="flex flex-col gap-5 flex-1 relative z-10">
                  {['Enter a topic or paste a link — get a full post','AI writes the hook, structures slides, fills in copy','Get a ready-to-publish post in one click','Instantly generate multiple variations to test','Your tone of voice preserved in every slide'].map((t,i)=>(
            ...
            <div className="flex flex-col items-center justify-center gap-3 w-full">
              ...
              <p className="text-sm text-zinc-500 text-center">First post in 60 seconds</p>
            </div>
          </div>
        </div>
      </motion.div>
...
const ProgressMockup = () => {
  ...
  return (
    <div className="mt-6 w-full max-w-xs flex flex-col gap-2">
      <div className="flex items-center justify-between"><span className="text-[11px] text-zinc-500 font-medium tracking-wide">Building post...</span><span className="text-[11px] text-zinc-400 font-bold tabular-nums">{pct}%</span></div>
...
const ExportMockup = () => (
  <div className="mt-6">
    <button className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-pink-500/50 bg-pink-500/10 shadow-[0_0_20px_rgba(236,72,153,0.2)] text-pink-300 text-xs font-semibold tracking-wide transition-all hover:shadow-[0_0_28px_rgba(236,72,153,0.4)] group">
      <Download className="w-3.5 h-3.5 group-hover:-translate-y-0.5 transition-transform duration-300"/> Download post
    </button>
  </div>
);

const hiwSteps = [
  { icon: CornerDownLeft, number: 1, title: 'Enter idea or paste link', desc: 'Drop a topic, URL, video, or raw idea. GoToFlow reads the context and preps your post.', micro: <InputMockup/> },
  { icon: Sparkles, number: 2, title: 'AI builds your post', desc: 'The AI writes a scroll-stopping hook, structures every slide, and fills in the copy — in seconds.', micro: <ProgressMockup/> },
  { icon: Download, number: 3, title: 'Get a ready post', desc: 'Review, tweak in the built-in editor, and download your ready-to-publish post.', micro: <ExportMockup/> },
];

export const PostHowItWorks = () => {
  const isMobile = useIsMobile();
  return (
  <section className="py-24 md:py-32 px-6 relative z-10 w-full overflow-hidden bg-black">
    ...
    <div className="max-w-7xl mx-auto bg-[#050505]/60 border border-white/[0.08] rounded-[2.5rem] p-8 md:p-12 lg:p-16 backdrop-blur-2xl relative z-10 shadow-[0_30px_100px_-15px_rgba(0,0,0,1),0_0_40px_rgba(236,72,153,0.15)]">
      <motion.div initial={{opacity:0,y:40}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration: isMobile ? 0.6 : 0.8}} className="text-center mb-16 flex flex-col items-center">
        ...
        <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-6 tracking-tight leading-tight text-balance">From raw idea to published post in <span className="text-gradient-brand">3 simple steps</span></h2>
    ...
    <motion.div initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.7,delay}} className={`relative group transition-transform duration-500 hover:-translate-y-1 transform-gpu ${className}`}>
      ...
      <div className="relative z-10 p-2 md:p-3 bg-[#ffffff03] backdrop-blur-2xl border border-white/[0.08] shadow-[0_20px_50px_rgba(0,0,0,0.5)] rounded-[1.25rem] flex flex-col justify-center h-full group-hover:border-white/[0.14] transition-colors duration-500">
        <div className="relative rounded-[0.75rem] overflow-hidden border border-white/[0.03] bg-[#0c0508] flex-grow flex items-center justify-center">
          {!err ? <img src={`/images/screens/screen-${imageId}.png`} alt={`GoToFlow AI post generator interface screen ${imageId}`} className="w-full h-auto block object-contain transition-transform duration-700 group-hover:scale-[1.02]" onError={()=>setErr(true)}/> :
...
const diffPoints = [
  { icon: Fingerprint, title: 'Your style, not generic AI text', desc: 'GoToFlow preserves your tone of voice and visual delivery, so every post sounds like you wrote it.' },
  ...
  { icon: Zap, title: 'Complete post, ready to publish', desc: 'Get a complete, structured post — not unfinished text you still need to split into slides manually.' },
];

export const PostDifferentiation = () => {
  const isMobile = useIsMobile();
  return (
    ...
    <div className="max-w-7xl mx-auto relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-20 items-center">
      <div className="flex flex-col">
        <motion.div initial={{opacity:0,x: isMobile ? 0 : -40, y: isMobile ? 24 : 0}} whileInView={{opacity:1,x:0,y:0}} viewport={{once:true}} transition={{duration: isMobile ? 0.6 : 0.8}}>
          ...
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-white mb-5 leading-[1.15] text-balance">GoToFlow <span className="text-gradient-brand">creates the full post workflow</span></h2>
          <p className="text-base md:text-lg text-zinc-400 font-medium leading-relaxed text-balance mb-12">Canva starts from a blank canvas and template. GoToFlow starts from your idea or source material and builds the hook, structure, copy, visual direction, CTA, and ready social post in one workflow.</p>
        </motion.div>
...
/* ── SEO Text Block ── */
export const PostSEOBlock = () => (
  <section className="py-20 md:py-28 px-6 relative z-10 w-full bg-[#050505]">
    <div className="max-w-3xl mx-auto">
      <h2 className="text-2xl md:text-3xl font-bold text-white tracking-tight mb-6">What is an AI LinkedIn Post Generator?</h2>
      <div className="text-zinc-400 leading-relaxed space-y-4 text-base">
        <p>An <strong className="text-zinc-200">AI post generator</strong> is a tool that uses artificial intelligence to automatically generate multi-slide social media posts — commonly known as posts — for platforms like LinkedIn, LinkedIn, and Telegram. Instead of writing each slide manually, you provide a topic or idea, and the AI produces a structured, hook-led post ready to publish.</p>
        <p>GoToFlow's <strong className="text-zinc-200">LinkedIn post</strong> generator goes beyond simple templates. It understands context, applies proven content frameworks, and crafts each slide with a clear purpose — from a scroll-stopping opener to a closing call-to-action.</p>
        <p>The benefits of <strong className="text-zinc-200">AI content</strong> generation for posts are significant: faster production, consistent quality, and the ability to test multiple angles without extra effort. Whether you're a creator, marketer, or founder, an AI post generator removes the hardest part of social media — knowing what to say.</p>
      </div>
    </div>
  </section>
);
...
/* ── FAQ ── */
const faqItems = [
  { q: 'What is an AI post generator?', a: 'An AI post generator is a tool that automatically generates multi-slide social media posts using artificial intelligence. You provide a topic, and the AI writes the hook, structures the slides, and fills each one with content — in seconds.' },
  { q: 'Can AI create LinkedIn posts?', a: 'Yes. GoToFlow generates post content optimized for LinkedIn, LinkedIn, and Telegram. The AI builds the hook, structure, copy, visual direction, and CTA so the result is a complete content workflow.' },
  { q: 'Is GoToFlow better than Canva for posts?', a: 'They solve different problems. Canva is a manual design editor. GoToFlow is an end-to-end content workflow: idea, structure, copy, visual direction, ready output, and CTA. Some users still use Canva for manual edits, but GoToFlow is not just a text layer.' },
  { q: 'How long does it take to generate a post?', a: 'Under 60 seconds. Input your idea or paste a URL, and GoToFlow delivers a full post structure with copy for every slide.' },
  { q: 'Do I need to be a good writer to use GoToFlow?', a: 'No. GoToFlow is designed for people who want great post content without being professional copywriters. Just describe your topic and the AI handles the rest.' },
];
    ...
    <section className="py-24 md:py-32 px-6 relative z-10 w-full overflow-hidden bg-[#050505]">
      <div className="max-w-7xl mx-auto bg-white/[0.02] border border-white/[0.05] rounded-[2.5rem] p-8 md:p-12 lg:p-16 backdrop-blur-sm relative z-10">
        <motion.div initial={{opacity:0,y:40}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration: isMobile ? 0.6 : 0.8}} className="text-center mb-14 flex flex-col items-center">
          ...
          <p className="text-base md:text-lg text-zinc-400 max-w-xl leading-relaxed text-balance">Everything you need to know about AI post creation</p>
        </motion.div>
          ...
          <div className="group relative rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden border border-white/[0.08] hover:border-white/[0.12] transition-colors duration-500" style={{background:'linear-gradient(180deg, rgba(255,255,255,0.04) 0%, rgba(255,255,255,0.015) 100%)',backdropFilter:'blur(40px) saturate(1.2)',boxShadow:'0 0 30px rgba(236,72,153,0.04), 0 40px 80px -25px rgba(0,0,0,0.8)'}}>
            <div className="relative z-10 px-8 py-16 sm:px-12 sm:py-20 md:px-20 md:py-24 lg:px-28 lg:py-28 flex flex-col items-center text-center">
              ...
              <h2 className="text-[1.6rem] sm:text-[2rem] md:text-[2.6rem] lg:text-[3.1rem] font-bold text-white tracking-[-0.035em] leading-[1.12] mb-6 max-w-2xl">
                Stop struggling with<br/>post <span className="text-gradient-brand">content.</span><br/><span className="text-zinc-400 font-semibold" style={{fontSize:'0.78em'}}>Start creating.</span>
              </h2>
              <p className="text-sm md:text-[0.92rem] text-zinc-500 max-w-lg leading-[1.75] font-medium mb-12">Join thousands of creators and entrepreneurs who create scroll-stopping posts in seconds with GoToFlow AI.</p>
              <div className="relative group mb-5">
                <button onClick={()=>window.location.href = getAppUrlWithRef(CTA_URL)} onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)} className="relative z-10 flex items-center justify-center gap-2.5 px-9 py-3.5 rounded-[14px] font-semibold text-white text-[15px] overflow-hidden cursor-pointer" style={{background:'linear-gradient(135deg, #ec4899 0%, #f97316 100%)',border:'1px solid rgba(255,255,255,0.18)',boxShadow:hover?'0 14px 55px rgba(236,72,153,0.5), inset 0 1px 0 rgba(255,255,255,0.25)':'0 8px 35px rgba(236,72,153,0.25), inset 0 1px 0 rgba(255,255,255,0.15)',transform:hover?'translateY(-2px) scale(1.04)':'translateY(0) scale(1)',transition:'all 0.4s cubic-bezier(0.25, 1, 0.5, 1)'}}>
                  <span className="relative z-30 tracking-[0.01em]">Create Your Post</span>
  ...
  <MainLayout>
    ...
    <section className="py-6 px-6 bg-[#050505] relative z-10 w-full flex justify-center">
      <div className="max-w-3xl w-full p-6 md:p-8 rounded-2xl border border-white/[0.05] bg-white/[0.02]">
        <h3 className="text-white font-medium mb-4 text-base md:text-lg">Expand your content formats:</h3>
        <ul className="space-y-3 text-sm md:text-base">
            ...
            <li className="flex items-center gap-2"><span className="text-pink-500">•</span><Link to="/ai-content-generator" className="text-zinc-300 hover:text-pink-400 transition-colors underline underline-offset-4 decoration-white/10 hover:decoration-pink-400/50">Generate full content</Link></li>
        </ul>
      </div>
    </section>
    <TestimonialsSection />
