...
export const SEOHeadRu = () => {
  useEffect(() => {
    document.title = 'ИИ-генератор постов для LinkedIn | Создать пост с ИИ';
    ...
    <div className="max-w-7xl mx-auto flex flex-col items-center text-center relative z-10 w-full">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="mt-4 md:mt-0 mb-8">
        <div className="inline-flex items-center gap-2.5 rounded-full border border-white/10 bg-white/5 px-4 py-1.5 backdrop-blur-sm">
          ...
          <span className="text-sm text-zinc-300 whitespace-nowrap">Генератор постов для LinkedIn</span>
        </div>
      </motion.div>
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: isMobile ? 0.6 : 0.8, delay: 0.1 }} className="max-w-3xl mx-auto w-full">
        <h1 className="text-[1.6rem] sm:text-[2rem] md:text-[2.6rem] lg:text-[3.1rem] font-bold text-white tracking-[-0.035em] leading-[1.12] mb-8 text-balance">
          Генератор постов для LinkedIn с ИИ —<br className="hidden md:block" /> <span className="text-gradient-brand">готовый пост за 60 секунд</span>
        </h1>
      </motion.div>
...
/* ── Showcase (RU) ── */
const carouselCards = [
  { id: 2, likes: '8.1K', views: '20K', label: 'Пост пример 2', image: '/images/niches/ru/content-ru-2.webp' },
  { id: 3, likes: '1.8K', views: '5K', label: 'Пост пример 3', image: '/images/niches/ru/content-ru-3.webp' },
  { id: 5, likes: '6.7K', views: '18K', label: 'Пост пример 5', image: '/images/niches/ru/content-ru-5.webp' },
  { id: 6, likes: '5.9K', views: '14K', label: 'Пост пример 6', image: '/images/niches/ru/content-ru-6.webp' },
  { id: 7, likes: '2.4K', views: '7K', label: 'Пост пример 7', image: '/images/niches/ru/content-ru-7.webp' },
  { id: 8, likes: '980', views: '3.2K', label: 'Пост пример 8', image: '/images/niches/ru/content-ru-8.webp' },
  { id: 9, likes: '10K', views: '25K', label: 'Пост пример 9', image: '/images/niches/ru/content-ru-9.webp' },
  { id: 10, likes: '7.2K', views: '15K', label: 'Пост пример 10', image: '/images/niches/ru/content-ru-10.webp' },
];
...
const SlideCard = ({ card }) => (
  <div className="shrink-0 w-[280px] md:w-[320px] bg-white/[0.02] border border-white/[0.06] rounded-2xl p-3 flex flex-col gap-3">
    <div className="relative w-full aspect-[4/5] rounded-xl bg-[#111] overflow-hidden">
      <img src={card.image} alt={`Пост LinkedIn: ${card.label}`} className="absolute inset-0 w-full h-full object-cover z-0" loading="lazy" />
        ...
        <div className="relative rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-12 lg:p-14">
          ...
          <div className="relative z-10 flex flex-col">
            ...
            <div className="flex flex-col items-center justify-center gap-3 w-full">
              ...
              <p className="text-sm text-zinc-500 text-center">Первый пост за 60 секунд</p>
            </div>
          </div>
        </div>
      </motion.div>
  ...
  <section className="py-20 md:py-28 px-6 relative z-10 w-full bg-[#050505]">
    <div className="max-w-3xl mx-auto">
      ...
      <div className="text-zinc-400 leading-relaxed space-y-4 text-base">
        <p>Генератор постов с ИИ — это инструмент, который автоматически создает <strong className="text-zinc-200">пост LinkedIn</strong> и других соцсетей. Вместо того чтобы писать каждый слайд вручную, вы просто вводите тему или идею, и ИИ выдает структурированный пост с сильным хуком, готовый к публикации.</p>
...
const faqItems = [
  ...
  { q: 'Есть ли специальный формат для LinkedIn?', a: <>Для LinkedIn отлично подходят PDF-документы. Вы можете использовать наш <a href="/ru/generator-karuselej-linkedin" className="text-pink-400 hover:underline">генератор каруселей для LinkedIn</a>, чтобы быстро создать экспертный PDF-файл.</> },
...
export const LinkedInPostPageRu = () => (
  <MainLayout>
    <SEOHeadRu />
    <Header />
    <CarouselHeroRu />
    ...
    <section className="py-6 px-6 bg-[#050505] relative z-10 w-full flex justify-center">
      <div className="max-w-3xl w-full p-6 md:p-8 rounded-2xl border border-white/[0.05] bg-white/[0.02]">
        ...
        <ul className="space-y-3 text-sm md:text-base">
            <li className="flex items-center gap-2"><span className="text-pink-500">•</span><Link to="/ru/ii-generator-karuseley" className="text-zinc-300 hover:text-pink-400 transition-colors underline underline-offset-4 decoration-white/10 hover:decoration-pink-400/50">Карусели</Link></li>
