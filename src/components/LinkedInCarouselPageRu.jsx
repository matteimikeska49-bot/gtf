...
const linkedInCarouselFormatsPage = {
  ...
  categoryCta: {
    label: 'Создать карусель LinkedIn',
    href: CTA_URL,
  },
};
...
export const SEOHeadRu = () => {
  useEffect(() => {
    document.title = 'Генератор каруселей LinkedIn с AI — идеи и структура постов | GoToFlow';
...
/* ── Showcase (RU) ── */
const carouselCards = [
  { id: 2, likes: '8.1K', views: '20K', label: 'Карусель пример 2', image: '/images/niches/ru/content-ru-2.webp' },
  { id: 3, likes: '1.8K', views: '5K', label: 'Карусель пример 3', image: '/images/niches/ru/content-ru-3.webp' },
  { id: 5, likes: '6.7K', views: '18K', label: 'Карусель пример 5', image: '/images/niches/ru/content-ru-5.webp' },
  { id: 6, likes: '5.9K', views: '14K', label: 'Карусель пример 6', image: '/images/niches/ru/content-ru-6.webp' },
  { id: 7, likes: '2.4K', views: '7K', label: 'Карусель пример 7', image: '/images/niches/ru/content-ru-7.webp' },
  { id: 8, likes: '980', views: '3.2K', label: 'Карусель пример 8', image: '/images/niches/ru/content-ru-8.webp' },
  { id: 9, likes: '10K', views: '25K', label: 'Карусель пример 9', image: '/images/niches/ru/content-ru-9.webp' },
  { id: 10, likes: '7.2K', views: '15K', label: 'Карусель пример 10', image: '/images/niches/ru/content-ru-10.webp' },
];
...
const SlideCard = ({ card }) => (
  <div className="shrink-0 w-[280px] md:w-[320px] bg-white/[0.02] border border-white/[0.06] rounded-2xl p-3 flex flex-col gap-3">
    <div className="relative w-full aspect-[4/5] rounded-xl bg-[#111] overflow-hidden">
      <img src={card.image} alt={`Карусель LinkedIn: ${card.label}`} className="absolute inset-0 w-full h-full object-cover z-0" loading="lazy" />
...
export const LICarouselShowcaseRu = () => (
...
/* ── Problem (RU) ── */
const problemCards = [
  { title: "Тратите 3 часа → одна карусель", text: "Написать хуки, придумать структуру слайдов и подготовить текст занимает много времени — особенно если результат обычного ИИ приходится полностью переписывать." },
...
const hiwSteps = [
  { icon: CornerDownLeft, number: 1, title: 'Введите тему или ссылку', desc: 'Введите тему, URL или идею. ИИ изучит контекст и подготовит карусель.', micro: <InputMockup/> },
  { icon: Sparkles, number: 2, title: 'ИИ создаёт карусель', desc: 'ИИ собирает хук, структуру, текст, визуальную подачу и CTA.', micro: <ProgressMockup/> },
    ...
    <motion.div initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.7,delay}} className={`relative group transition-transform duration-500 hover:-translate-y-1 transform-gpu ${className}`}>
      ...
      <div className="relative z-10 p-2 md:p-3 bg-[#ffffff03] backdrop-blur-2xl border border-white/[0.08] shadow-[0_20px_50px_rgba(0,0,0,0.5)] rounded-[1.25rem] flex flex-col justify-center h-full group-hover:border-white/[0.14] transition-colors duration-500">
        <div className="relative rounded-[0.75rem] overflow-hidden border border-white/[0.03] bg-[#0c0508] flex-grow flex items-center justify-center">
          {!err ? <img src={`/images/screens/screen-${imageId}.png`} alt={`GoToFlow AI генератор каруселей экран ${imageId}`} className="w-full h-auto block object-contain transition-transform duration-700 group-hover:scale-[1.02]" onError={()=>setErr(true)}/> :
...
const diffPoints = [
  { icon: Fingerprint, title: 'Ваш стиль, а не шаблонный текст', desc: 'GoToFlow сохраняет ваш tone of voice, поэтому каждая карусель звучит так, будто её написали вы.' },
...
export const LICarouselSEOBlockRu = () => (
  <section className="py-20 md:py-28 px-6 relative z-10 w-full bg-[#050505]">
    <div className="max-w-3xl mx-auto">
      <h2 className="text-2xl md:text-3xl font-bold text-white tracking-tight mb-6">Что такое генератор каруселей с ИИ</h2>
      <div className="text-zinc-400 leading-relaxed space-y-4 text-base">
        <p>Генератор каруселей с ИИ — это инструмент, который автоматически создает <strong className="text-zinc-200">карусель LinkedIn</strong> и других соцсетей. Вместо того чтобы писать каждый слайд вручную, вы просто вводите тему или идею, и ИИ выдает структурированную карусель с сильным хуком, готовую к публикации.</p>
        <p>Наш генератор каруселей работает не как обычные шаблоны. ИИ анализирует контекст, применяет проверенные контентные фреймворки и <strong className="text-zinc-200">создает карусель</strong> с четкой целью для каждого слайда — от цепляющего заголовка до призыва к действию (CTA) в конце.</p>
          ...
          <div className="group relative rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden border border-white/[0.08] hover:border-white/[0.12] transition-colors duration-500" style={{background:'linear-gradient(180deg, rgba(255,255,255,0.04) 0%, rgba(255,255,255,0.015) 100%)',backdropFilter:'blur(40px) saturate(1.2)',boxShadow:'0 0 30px rgba(236,72,153,0.04), 0 40px 80px -25px rgba(0,0,0,0.8)'}}>
            <div className="relative z-10 px-8 py-16 sm:px-12 sm:py-20 md:px-20 md:py-24 lg:px-28 lg:py-28 flex flex-col items-center text-center">
              ...
              <h2 className="text-[1.6rem] sm:text-[2rem] md:text-[2.6rem] lg:text-[3.1rem] font-bold text-white tracking-[-0.035em] leading-[1.12] mb-6 max-w-2xl">
                Готовая карусель за 60 секунд. <span className="text-gradient-brand">Без дизайна.</span><br/><span className="text-zinc-400 font-semibold" style={{fontSize:'0.78em'}}>Без команды.</span>
              </h2>
...
export const LinkedInCarouselPageRu = () => (
  <MainLayout>
    <SEOHeadRu />
    <Header />
    <LICarouselHeroRu />
    <LICarouselProblemRu />
    <LICarouselComparisonRu />
