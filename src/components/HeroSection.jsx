  ...
  return (
    ...
    >
      ...
      <div className="relative rounded-2xl md:rounded-[2.5rem] border border-white/5 bg-[#0a0a0a]/80 backdrop-blur-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row text-left ring-1 ring-white/5 group md:aspect-auto">
        ...
        {/* ═══════════════════════════════════════════ */}
        <div className="hidden md:flex flex-1 p-8 lg:p-10 flex-col gap-6 relative z-10 bg-[#0a0a0a]/30 overflow-hidden">
          ...
          <div className="flex gap-2.5 relative z-10">
            {tabLabels.map((label, i) => (
              <div
                key={label}
                className={`px-4 py-2 rounded-lg text-xs font-semibold tracking-wide transition-all duration-500 border ${
                  activeTab === i
                    ? 'bg-gradient-to-r from-pink-500/15 to-orange-500/10 border-pink-500/30 text-pink-300 shadow-[0_0_14px_rgba(236,72,153,0.15)]'
          ...
          {/* Main content grid */}
          <div className="grid grid-cols-3 gap-6 relative z-10 flex-1">

            {/* Left: generation preview */}
            <div className="col-span-2 p-6 rounded-[1.5rem] border border-white/[0.06] bg-[#050505]/80 shadow-xl relative overflow-hidden flex flex-col gap-5">
              <div className="absolute inset-0 bg-gradient-to-br from-white/[0.015] to-transparent pointer-events-none" />

              {/* Input */}
              <div className="relative z-10">
                <div className="text-[10px] uppercase tracking-[0.14em] text-zinc-600 font-bold mb-2">{isRu ? 'Запрос' : 'Input'}</div>
              ...
              <div className="relative z-10 flex items-center gap-3 px-1">
                ...
                <Sparkles className="w-3.5 h-3.5 text-pink-400/60 animate-pulse" />
                <div className="h-px flex-1 bg-gradient-to-r from-transparent via-white/[0.06] to-transparent" />
              </div>

              {/* Output */}
              <div className="relative z-10 flex-1">
                <div className="text-[10px] uppercase tracking-[0.14em] text-zinc-600 font-bold mb-2">{isRu ? 'Результат' : 'Generated'}</div>
                <div className="flex flex-col gap-2 px-4 py-3 rounded-xl bg-white/[0.03] border border-white/[0.08]">
                  <AnimatePresence mode="wait">
                    <motion.div
                      key={activeTab}
                    ...
                    >
                      {activeTab === 0 && (
                        <>
                          <span className="text-sm text-zinc-300 font-medium">{isRu ? '✓ Хук + 3 контентных блока' : '✓ Hook + 3 content blocks'}</span>
                          <span className="text-xs text-zinc-500">{isRu ? 'Структура поста + призыв к действию готовы' : 'Post structure + CTA ready'}</span>
                        </>
                      )}
                      {activeTab === 1 && (
                        <>
                          <span className="text-sm text-zinc-300 font-medium">{isRu ? '✓ 8 слайдов + дизайн обложки' : '✓ 8 slides + cover design'}</span>
                          <span className="text-xs text-zinc-500">{isRu ? 'Текст карусели + визуальный макет' : 'Carousel copy + visual layout'}</span>
                        </>
                      )}
                      {activeTab === 2 && (
                        <>
                          <span className="text-sm text-zinc-300 font-medium">{isRu ? '✓ 3 хука + структура сценария' : '✓ 3 hooks + script outline'}</span>
                          <span className="text-xs text-zinc-500">{isRu ? 'Структура Reels + подписи' : 'Reel structure + captions'}</span>
                        </>
                      )}
                    </motion.div>
                  </AnimatePresence>
              ...
              {/* AI generation bar */}
              <div className="relative z-10 p-3 rounded-xl border border-pink-500/20 bg-gradient-to-r from-pink-500/10 to-orange-500/5 flex items-center gap-3 overflow-hidden">
                 ...
                 <Sparkles className="w-4 h-4 text-pink-400 shrink-0 relative z-10" />
            ...
            {/* Right: analytics card (preserved) */}
