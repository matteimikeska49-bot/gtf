...
const cliSlugs = process.argv.slice(2).filter((arg) => !arg.startsWith('--'));
...
const candidateTopics = topicMap.filter((topic) => {
...
for (const topic of candidateTopics) {
  ...
  if (!keyword && !needsRefresh) packageBlockers.push('missing keyword record');
  ...
  if (topic.mockupRequired === undefined && topic.mockupStatus === undefined) packageBlockers.push('missing mockup decision');
  if (topic.mockupRequired === true && (!topic.mockupSlots || topic.mockupSlots.length === 0)) packageBlockers.push('mockupRequired true but mockupSlots empty');
  if (!topic.approvalStatus && topic.decision !== 'approve') packageBlockers.push('missing approvalStatus/approval decision');
  ...
  if (topic.mockupStatus === 'not_available' || score?.scores?.mockupReadinessScore <= 5) packageWarnings.push('mockup weak/not available');
  ...
  const researchPackage = {
    ...
    mockupRequired: topic.mockupRequired ?? null,
    mockupSlots: topic.mockupSlots || topic.requiredMockupSlots || [],
    ...
    approvalStatus: topic.approvalStatus || topic.decision || null
  };
